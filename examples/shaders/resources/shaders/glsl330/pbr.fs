...
vec3 ComputePBR() {
    ...
    vec3  albedo    = texture(albedoMap, uv).rgb * albedoColor.rgb;
    float metallic  = clamp(metallicValue,  0.0, 1.0);
    float roughness = clamp(roughnessValue, 0.0, 1.0);
    float ao        = clamp(aoValue,        0.0, 1.0);
    
    if (useTexMRA == 1) {
        vec4 mra  = texture(mraMap, uv) * useTexMRA;
    ...
    // emissive = (texture(emissiveMap, uv).rgb).g * emissiveColor.rgb*emissivePower * useTexEmissive;
    if (useTexEmissive == 1) {
        emissive = texture(emissiveMap, uv).r * emissiveColor.rgb * emissivePower * useTexEmissive;
        // emissive = vec3(texture(emissiveMap, uv).b);
    }

    // return N;//vec3(metallic,metallic,metallic);
    // if dia-electric use base reflectivity of 0.04 otherwise ut is a metal use albedo as base reflectivity
    vec3 baseRefl   = mix(vec3(0.04), albedo.rgb, metallic);
    vec3 lightAccum = vec3(0);  // Acumulate lighting lum

    int enabledLights = 0;
    for (int i = 0; i < numOfLights; i++) {
        Light light = lights[i];

        if (light.enabled != 0) { enabledLights++; }
    
        vec3  L           = normalize(light.position - fragPosition);    // Compute light vector
        vec3  H           = normalize(V + L);                            // Compute halfway bisecting vector
        float dist        = length(light.position - fragPosition);       // Compute distance to light
        float attenuation = 1.0/(dist*dist*0.23);                        // Compute attenuation
        vec3  radiance    = light.color.rgb*light.intensity*attenuation; // Compute input radiance, light energy comming in
    ...
    vec3 ambientFinal = (ambientColor + albedo) * ambient * (0.1 * enabledLights + 0.01);
